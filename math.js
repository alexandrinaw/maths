...
function orderOfOps(input) {
    ...
    if (input.length === 1) {
        if (Array.isArray(first)) return orderOfOps(first);
        else return [first];
    }
    else if (input.length === 2 && first === "-") {
        first = orderOfOps(["0", first, op1]);
        return first;
    }
    else if (input.length === 3) {
        if (Array.isArray(first) && first.length !== 1) first = orderOfOps(first);
        if (Array.isArray(second) && second.length !== 1) second = orderOfOps(second);
    ...
    if (compOps(op1, op2) < 0) {
        return ([first].concat(orderOfOps(op1)).concat([orderOfOps(input.slice(2))]));
    }
    else {
        var element = orderOfOps([first].concat([op1, second]));
...
function doMath(nestedMath) {
    ...
    if (operator === undefined) return operandA;
    var result = operatorFns[operator](parseFloat(operandA), parseFloat(operandB));
    if (nestedMath.length > 3) result = doMath([result].concat(nestedMath.slice(3)));
    return result;
}
